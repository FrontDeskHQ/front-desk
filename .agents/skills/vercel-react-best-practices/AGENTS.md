...
Comprehensive performance optimization guide for React applications, designed for AI agents and LLMs. Contains 40+ rules across 7 categories, prioritized by impact from critical (eliminating waterfalls, reducing bundle size) to incremental (advanced patterns). Each rule includes detailed explanations, real-world examples comparing incorrect vs. correct implementations, and specific impact metrics to guide automated refactoring and code generation.
...
In HTTP handlers (API routes, serverless functions, or other server endpoints), start independent operations immediately, even if you don't await them yet.
...
Some frameworks provide helpers (for example Next.js `next/script` with a `strategy` prop) that wrap these semantics; prefer those when the framework documents them.
