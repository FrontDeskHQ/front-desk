...
- **Silent re-enable schedules the connector's own catch-up.** Liveness only answers "does the install still exist" — it says nothing about what the mirror missed. On `live: true` the core also kicks off the connector's existing catch-up path (github: one idempotent repo backfill per connected repo, the same jobs the install callback enqueues). This is a safety net, not the primary freshness mechanism: webhook handlers don't gate on `integration.enabled`, so a plain disable → enable cycle usually leaves the mirror current. It covers deliveries dropped while the connector was down and gives Enable an observable sync signal.
