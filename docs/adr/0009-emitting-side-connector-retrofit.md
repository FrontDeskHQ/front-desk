...
- **The framework owns author identity.** Ingest takes a neutral `author: { externalId, name, avatarUrl? }`; the API owns author-row find-or-create/dedup on `(organizationId, metaId)`, refreshes `name` when the provider sends a new one (a failed lookup, `Unknown`, never erases an existing name), and applies the `provider:` prefixing convention. Only the un-liftable provider call — resolving a raw user id → display name (Slack's async API lookup, Discord's `displayName`) — stays in the connector.
