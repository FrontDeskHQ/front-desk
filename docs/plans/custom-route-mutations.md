...
| `thread` | `acceptRead`, `dismissRead`, `acceptInlineSuggestion`, `dismissInlineSuggestion`, `setStatus`, `setPriority`, `assignUser`, `linkGithubIssue`, `unlinkGithubIssue` | Generic `thread.update` remains for PR/archive; `create` also lacks optimistic UI |
...
| [x] **LP-003b** | `thread.linkGithubIssue` / `unlinkGithubIssue` | `thread-mutations.ts`, `issues.tsx`, optimistic handlers | `issues.tsx` has zero `thread.update` + `update.insert` for issue link/unlink |
...
- 2026-06-21 (LP-003b): `runLinkGithubIssue` / `runUnlinkGithubIssue` resolve issue labels from `externalEntity` mirror; activity type `issue_changed` with same metadata shape as prior web writes. `createGithubIssue` success still calls `linkGithubIssue` separately (create procedure does not auto-link).
...
- 2026-06-21 (LP-003b): Implemented `thread.linkGithubIssue` / `unlinkGithubIssue` — `runLinkGithubIssue` / `runUnlinkGithubIssue` in `thread-mutations.ts`, procedures in `router/threads.ts`, migrated `issues.tsx`, optimistic handlers in `live-state.ts`.

## Handoff

Next action: Ship **LP-003c** — implement `thread.linkGithubPullRequest` / `unlinkGithubPullRequest` in `apps/api/src/lib/thread-mutations.ts`, migrate `apps/web/src/components/threads/pull-requests.tsx`, add optimistic handlers. Mirror LP-003b patterns (`externalPrId`, `pr_changed` activity type).
