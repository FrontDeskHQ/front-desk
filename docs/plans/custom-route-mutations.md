...
- Status: in-progress
- Active checkpoint: **LP-003d** (next PR slice)
- Branch or PR: none
...
**LP-003 progress:** Shared thread write helpers live in `apps/api/src/lib/thread-mutations.ts`. Implemented `thread.setStatus`, `thread.setPriority`, `thread.assignUser`, `thread.linkIssue`, `thread.unlinkIssue`, `thread.linkPullRequest`, `thread.unlinkPullRequest` (API procedures + web migration + optimistic handlers). `set-status` signal handler delegates to `runSetThreadStatus`. `issues.tsx` and `pull-requests.tsx` have zero generic `thread.update` + `update.insert` for issue/PR link/unlink. Remaining LP-003 work: `markDuplicate`, `archive`/`restore`, `setAgentRead`, slack/discord `thread.create` / generic `message.insert`, and other generic `thread.update` call sites (`archive/$id.tsx`, `threads/$id/index.tsx`, `quick-actions.tsx`, devtools).
...
| `thread` | `acceptRead`, `dismissRead`, `acceptInlineSuggestion`, `dismissInlineSuggestion`, `setStatus`, `setPriority`, `assignUser`, `linkIssue`, `unlinkIssue`, `linkPullRequest`, `unlinkPullRequest` | Generic `thread.update` remains for archive; `create` also lacks optimistic UI |
...
Next action: Ship **LP-003d** — implement `thread.markDuplicate` in `apps/api/src/lib/thread-mutations.ts`, migrate `apps/web/src/components/threads/quick-actions.tsx` duplicate accept path, refactor `apps/api/src/lib/signals/handlers/mark-duplicate.ts` to call shared helper, add optimistic handler. Mirror `setStatus` / issue-link patterns (`status` duplicate + `marked_duplicate` activity type).
