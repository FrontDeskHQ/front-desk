...
- 2026-06-24 (LP-008): `bun run typecheck` (root, 10 packages) pass. Ripgrep across `apps/api/src/live-state` confirms all product routes use `insert: () => false` and `update.preMutation/postMutation: () => false`; only `allowlist`, `subscription.update`, `pipelineIdempotencyKey`, `pipelineJob` allow internal-key generic writes. Ripgrep across `apps/`: zero product-route `mutate.*.(insert|update)`; only `fetchClient.mutate.pipeline*`. No runtime smoke test.
...
Next action: Ship **LP-009a** — repo-wide static verification. Run `bun run typecheck` and ripgrep for any remaining `mutate.<product>.insert` / `.update` under `apps/` (expect pipeline tables only). Record results in `Verification Ledger`; if clean, mark LP-009a complete and proceed to **LP-009b** smoke matrix.
