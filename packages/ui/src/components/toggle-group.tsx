...
function ToggleGroup({
  ...
  return (
    <ToggleGroupContext.Provider value={{ size, multiple }}>
      <BaseToggleGroup
...
function ToggleGroupItem({
  ...
  const { size, multiple } = React.useContext(ToggleGroupContext);
  ...
  return (
    <BaseToggle
      ...
      // onFocus={(event) => {
      //   onFocus?.(event);
      //   if (multiple || event.defaultPrevented) return;
      //   const target = event.currentTarget;
      //   if (target.disabled || target.dataset.pressed !== undefined) return;
      //   // Only auto-select on keyboard focus, not mouse clicks (which also focus).
